...
const initialState = {
  ...
  const BlueToothReducer = (state = initialState, action) => {
    switch (action.type) {
      ...
      // return { ...state, unpairedDevices : action.unpairedDevices.filter((d) => d.id !== action.device.id) }

      case types.CONNECTINGTRUE:
      return { ...state, connecting : true }
      case types.CONNECTDEVICE:
      return { ...state,  device:[...state.device,action.device], connected: true, connecting: false }
      case types.SECTION:
      ...
      case types.VALUE:
      return { ...state, isEnabled : action.value.isEnabled ,devices:action.value.devices  }
