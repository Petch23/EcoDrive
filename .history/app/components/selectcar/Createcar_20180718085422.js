...
export class CreateCar extends React.Component {
  ...
  AddData(){
    ...
    const car = [...Object.values(this.state.details)[0],Object.values(this.state.details)[0]];
    
    firebaseService.database().ref(`user/${uid}`).push().set({email:email,Cardata:car}).catch((error)=>{
    console.log(error);
  });
}
