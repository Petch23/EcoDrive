...
export class CreateCar extends React.Component {
  ...
  AddData(){
    ...
    const model = Object.values(this.state.cardetail)[1];
  console.log(typeof(checky));
  console.log(typeof(model));
  
    firebaseService.database().ref(`user/${uid}`).on('value',function(snapshot){ 
      snapshot.forEach(function(childSnapshot) {
        childSnapshot.forEach(function(childchildSnapshot){
            var childData = Object.values(childchildSnapshot.val())[1];
    ...
    return (
      ...
      <View style={{}}>
        ...
        {/* <List dataArray={this.state.seriecar} 
          ...
          <Text>{JSON.stringify([ ...new Set(Object.values(this.state.cardata)[0]) ])}</Text>
