...
export class CreateCar extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      checkcar:[],
      cardetail:[],
      checkbeforeadd:[], // เพิ่มรถครั้งแรกตอนที่ใน firebase ไม่มีรถ 
    };
  }
  onValueChange(value) {
    this.setState({
  ...
  AddData(){
    ...
    const checkbeforeadd = this.state.checkbeforeadd;
    console.log(checky)

  if(checky.includes(model) === false || checkbeforeadd.includes(model) === false) { //////เช็คว่าเคยเพิ่มรถคันนี้ไปยัง
    firebaseService.database().ref(`user/${uid}`).push({
      car
    }) .catch((error)=>{
    console.log(error);
  });
  this.setState({checkbeforeadd:[...this.state.checkbeforeadd,model]})
      }else{
        console.log('555')
      }
  ...
  render() {
    return (
      <View style={{}}>
          ...
          <Text>{JSON.stringify(this.state.checkbeforeadd)}</Text>
