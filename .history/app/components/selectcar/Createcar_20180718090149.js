...
export class CreateCar extends React.Component {
  ...
  AddData(){
    ...
    const car = Object.values(this.state.details);
    this.setState({addcar: [...this.state.addcar,car]})
    
    firebaseService.database().ref(`user/${uid}`).set({email:email,Cardata:this.state.addcar}).catch((error)=>{
    console.log(error);
  });
}
