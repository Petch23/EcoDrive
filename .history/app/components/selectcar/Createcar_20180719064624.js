...
export class CreateCar extends React.Component {
  ...
  AddData(){
    ...
    const check = (Object.values(this.state.checkcar)[0])
    firebaseService.database().ref(`user/${uid}`).once('value',function(snapshot){ //
      const check = snapshot.val(); //
      this.setState({ checkcar: [check] }) //
      console.log(Object.values(this.state.checkcar)[0].Cardata.Model) //
    }.bind(this), function(error) { console.log(error); }); //
    
    if(this.state.details.filter((i) => i.Model) != check.Cardata.Model) {//
