...
export class CreateCar extends React.Component {
  ...
  AddData(){
    ...
    const carmodel = Object.values(this.state.cardetail)[1];
    const checky = this.state.checkcar
    const id = firebaseService.database().ref(`user/${uid}`).push().getKey();
    // console.log(id)
    ...
    firebaseService.database().ref(`user/${uid}`).once('value',function(snapshot){ 
      snapshot.forEach(function(childSnapshot) {
        childSnapshot.forEach(function(childchildSnapshot){
          const childData = childchildSnapshot.val(); 
      ...
      // this.setState({ checkcar: check }) 
    }.bind(this), function(error) { console.log(error); }); //
    
    // if(this.state.cardetail.filter((i) => Object.values(i)[1])) {//
