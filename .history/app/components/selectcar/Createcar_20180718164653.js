...
export class CreateCar extends React.Component {
  ...
  CarDetail () {
    ...
    firebaseService.database().ref(`CarList/${make}/${model}/${series}`).onne('value',function(snapshot){
      const detail = snapshot.val();
      this.setState({ details: [detail] })
      this.setState({addcar: [...this.state.addcar,detail]})
    }.bind(this), function(error) { console.log(error); });
  }
  }
  ...
  AddData(){
    ...
    const car = Object.values(this.state.addcar);

    firebaseService.database().ref(`user/${uid}`).push().update({
      Cardata:car
    }).catch((error)=>{
    console.log(error);
  });
    ...
    return (
      ...
      <View style={{}}>
        ...
        {/* <List dataArray={this.state.seriecar} 
          ...
          <Text>{JSON.stringify([...new Set(this.state.cardata)])}</Text>
