 ...
 const DeviceList = ({ devices, connectedId, showConnectedIcon, onDevicePress }) =>
  <ScrollView style={styles.container}>
    <View style={styles.listContainer}>
      {devices.map((`${this.props.device}`, i) => {
        return (
          <TouchableHighlight
            underlayColor='#DDDDDD'
            key={`${this.props.device.id}_${i}`}
            style={styles.listItem} onPress={() => onDevicePress(`${this.props.device}`)}>
            <View style={{ flexDirection: 'row' }}>
              {showConnectedIcon
              ? (
                <View style={{ width: 48, height: 48, opacity: 0.4 }}>
                  {connectedId === `${this.props.device.id}`
                  ? (
              ...
              <View style={{ justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }}>
                <Text style={{ fontWeight: 'bold' }}>{`${this.props.device.name}`}</Text>
                <Text>{`<${this.props.device.id}>`}</Text>
              </View>
            </View>
          </TouchableHighlight>
        )
